...
async def right_answer(callback: types.CallbackQuery):
    ...
    if current_question_index < len(quiz_data):
        ...
        await callback.message.answer(
            f"Это был последний вопрос. Квиз завершен! Вы ответили правильно на {correct_answers} из {len(quiz_data)} вопросов."
        )

        # Сброс количества правильных ответов для пользователя
        await reset_correct_answers(callback.from_user.id)
